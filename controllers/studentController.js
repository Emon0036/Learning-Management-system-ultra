...
exports.dashboard = async (req, res) => {
  const [recentAttempts, availableQuizCount, completedCount, enrollments, progress, pendingReviewCount, availableCourseCount, myCourseCount] = await Promise.all([
    ...
    CourseEnrollment.countDocuments({ student: req.user._id, status: { $in: ['active', 'completed'] } }),
  ]);
  ...
  res.render('student/dashboard', {
    ...
    recentAttempts,
    stats: { availableQuizCount, completedCount, enrolledCount: enrollments.length, pendingReviewCount, availableCourseCount, myCourseCount },
    examTypeCounts,
    progress: progress || {
      totalPoints: 0,
      averageScore: 0,
