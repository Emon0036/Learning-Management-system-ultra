...
function buildQuestionPayload(quiz, body) {
  ...
  if (type === 'multiple-choice') {
    ...
    const correctAnswer = cleanText(body.correctAnswer);
    if (!correctAnswer) errors.push('Add the reference answer students should see after submission.');
    payload.options = [];
    payload.correctAnswer = correctAnswer;
  } else if (type === 'coding') {
    ...
    if (!correctAnswer) errors.push('Add the reference solution students should see after submission.');
    payload.correctAnswer = correctAnswer;
...
exports.dashboard = async (req, res) => {
  ...
  const [courseEnrollmentCount, upcomingSessionCount] = courseIds.length
    ? await Promise.all([
        CourseEnrollment.countDocuments({ course: { $in: courseIds } }),
        CourseSession.countDocuments({ course: { $in: courseIds }, status: 'scheduled', scheduledAt: { $gte: new Date() } }),
      ])
    : [0, 0];
...
exports.createQuiz = async (req, res) => {
  ...
  return res.status(201).render('teacher/quiz-form', {
    title: 'Edit Quiz',
    quiz,
    questions: [],
    action: `/teacher/quizzes/${quiz._id}?_method=PUT`,
    success: ['Quiz created. You can add questions below without leaving this builder.'],
    canonicalUrl: `/teacher/quizzes/${quiz._id}/edit${req.currentTabId ? `?tab=${encodeURIComponent(req.currentTabId)}` : ''}`,
  });
};
...
exports.updateReview = async (req, res) => {
  ...
  attempt.answers.forEach((answer, index) => {
    ...
    const referenceAnswer = teacherCorrectAnswer || cleanText(answer.teacherCorrectAnswer) || cleanText(answer.question.correctAnswer);

    if (!Number.isFinite(submittedMarks)) {
      reviewErrors.push(`Assign marks for question ${index + 1}.`);
    }
    if (isManualQuestion && !referenceAnswer) {
  ...
  attempt.answers.forEach((answer, index) => {
    ...
    if (isManualQuestion) {
      answer.teacherCorrectAnswer =
        cleanText(req.body.teacherCorrectAnswers?.[index]) ||
        cleanText(answer.teacherCorrectAnswer) ||
        cleanText(answer.question.correctAnswer);
    }
