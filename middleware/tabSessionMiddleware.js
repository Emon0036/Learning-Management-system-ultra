...
function getTabUsers(req) {
  return req.session?.tabUsers || {};
}

function getSingleTabId(req) {
  const tabIds = Object.keys(getTabUsers(req));
  return tabIds.length === 1 ? tabIds[0] : null;
}

function expectedRoleForRequest(req) {
  const requestPath = req.path || '';

  if (requestPath.startsWith('/student') || requestPath.startsWith('/enrollments')) return 'student';
  if (requestPath.startsWith('/teacher') || requestPath === '/auth/teacher-pending') return 'teacher';
  if (requestPath.startsWith('/admin') && requestPath !== '/admin/setup') return 'admin';

  return null;
}

async function findTabUserForRequestRole(req) {
  const expectedRole = expectedRoleForRequest(req);
  const tabEntries = Object.entries(getTabUsers(req));
  if (!expectedRole || !tabEntries.length) return null;

  const userIds = [...new Set(tabEntries.map(([, userId]) => String(userId)))];
  const users = await User.find({ _id: { $in: userIds }, role: expectedRole }).select('_id').lean();
  const matchingUserIds = new Set(users.map((user) => String(user._id)));
  const matchingTabs = tabEntries.filter(([, userId]) => matchingUserIds.has(String(userId)));

  if (!matchingTabs.length) return null;

  const lastActiveMatch = matchingTabs.find(([tabId]) => tabId === req.session?.lastActiveTabId);
  const matchingUserCount = new Set(matchingTabs.map(([, userId]) => String(userId))).size;
  const [tabId, userId] = lastActiveMatch || (matchingUserCount === 1 ? matchingTabs[0] : []);
  if (!tabId || !userId) return null;

  return { tabId, userId: String(userId) };
}
...
function canRecoverFromPassportSession(req) {
  ...
  if (req.currentTabId) {
    return !hasExistingTabUsers(req) || Boolean(req.session.tabUsers[req.currentTabId]);
  }
  if (getSingleTabId(req)) return true;
  return !hasExistingTabUsers(req);
}
...
function attachTabUser(req, res, next) {
  ...
  if (tabId && isGuestAuthRoute(req) && req.session.tabUsers[tabId]) {
...
async function resolveTabUser(req, res, next) {
  if (!req.currentTabId) {
    const routeTabUser = await findTabUserForRequestRole(req);
    if (routeTabUser) {
      req.currentTabId = routeTabUser.tabId;
      req.currentTabUserId = routeTabUser.userId;
    }
  }
  ...
  if (!req.currentTabId) {
    ...
    req.currentTabId = getSingleTabId(req) || req.session.lastActiveTabId || generateTabId();
  }
  ...
  if (!scopedUserId) {
    ...
    if (routeTabUser) {
      req.currentTabId = routeTabUser.tabId;
      req.currentTabUserId = routeTabUser.userId;
      scopedUserId = routeTabUser.userId;
    }
  }
